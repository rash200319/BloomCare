...
async def export_monthly_screening_trends():
    ...
    try:
        ...
        monthly_data = [
            {"month": "January", "screenings": 1245, "escalations": 187, "low_risk": 1058},
            {"month": "February", "screenings": 1380, "escalations": 207, "low_risk": 1173},
            {"month": "March", "screenings": 1520, "escalations": 228, "low_risk": 1292},
            {"month": "April", "screenings": 1650, "escalations": 264, "low_risk": 1386},
            {"month": "May", "screenings": 1890, "escalations": 302, "low_risk": 1588},
            {"month": "June", "screenings": 2100, "escalations": 336, "low_risk": 1764},
        ]
        ...
        elements.append(Paragraph("Monthly Screening Trends Report", heading_style))
        elements.append(Spacer(1, 0.3*inch))
        
        # Add metadata
        report_date = datetime.now().strftime("%B %d, %Y")
        elements.append(Paragraph(f"Report Generated: {report_date}", subtitle_style))
        elements.append(Spacer(1, 0.3*inch))
        ...
        avg_escalation_rate = (total_escalations / total_screenings * 100) if total_screenings > 0 else 0
        ...
        table_data = [['Month', 'Total Screenings', 'Escalations', 'Low Risk', 'Escalation %']]
        
        for row in monthly_data:
            escalation_pct = (row['escalations'] / row['screenings'] * 100) if row['screenings'] > 0 else 0
            table_data.append([
                row['month'],
                str(row['screenings']),
                str(row['escalations']),
        ...
        monthly_table = Table(table_data, colWidths=[1.5*inch, 1.5*inch, 1.2*inch, 1.2*inch, 1.2*inch])
