...
class Stage2Diagnostic(Base):
    ...
    stage1_screening_id = Column(String(36), ForeignKey("stage1_screenings.id", ondelete="SET NULL"))
    gestational_age_weeks = Column(Integer, nullable=False)
    
    # Disease being checked (from stage 1 high risk)
    primary_disease_checked = Column(String(50))  # e.g., "preeclampsia", "gdm", "preterm"
    ...
    stage1_screening = relationship("Stage1Screening", backref="stage2_diagnostics")
...
class Stage2Recommendation(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    patient_id = Column(String(36), ForeignKey("patients.id", ondelete="CASCADE"))
    stage1_screening_id = Column(String(36), ForeignKey("stage1_screenings.id", ondelete="CASCADE"))
    
    # Disease selected by doctor to check
    primary_disease_to_check = Column(String(50), nullable=False)  # e.g., "preeclampsia", "gdm", "preterm"
    ...
    stage1_screening = relationship("Stage1Screening", backref="stage2_recommendations")
...
class PatientReport(Base):
    ...
    patient_id = Column(String(36), ForeignKey("patients.id", ondelete="CASCADE"))
    stage1_screening_id = Column(String(36), ForeignKey("stage1_screenings.id", ondelete="CASCADE"))
    stage2_diagnostic_id = Column(String(36), ForeignKey("stage2_diagnostics.id", ondelete="SET NULL"))
    
    # Report type
    report_type = Column(String(50), nullable=False)  # "stage1", "stage2", "combined"
